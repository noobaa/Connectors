...
class ConfigHandler(splunk.admin.MConfigHandler):
    ...
    def handleList(self, confInfo):
        ...
        km = AwsAccessKeyManager(KEY_NAMESPACE, '-', self.getSessionKey())
        ret=km.all_accesskeys()
        logger.info("get acct : " + str(ret))

        filter_prefix = "_aws_"

        for c in ret:
            if str(c.name).lower().startswith(filter_prefix):
                continue
            confInfo[c.name].append('fname', c.name)
            confInfo[c.name].append('key_id', c.key_id)
            confInfo[c.name].append('delete', 0)
