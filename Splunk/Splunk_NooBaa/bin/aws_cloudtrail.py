...
class MyScript(smi.Script):
    ...
    def stream_events(self, inputs, ew):
        ...
        try:
            ...
            while not self._canceled:
                ...
                completed, keys_to_delete, failed, s3_notifications = self.process_CT_notifications(s3_conn, notifications)
                s3_completed, s3_keys_to_delete, s3_failed=self.process_S3_notifications(s3_conn, s3_notifications)
                completed.extend(s3_completed)
                failed.extend(s3_failed)
                for bucket in keys_to_delete:
                    if bucket in s3_keys_to_delete:
                        keys_to_delete[bucket].extend(s3_keys_to_delete[bucket])
                for bucket in s3_keys_to_delete:
                    if bucket not in keys_to_delete:
                        keys_to_delete[bucket]=s3_keys_to_delete[bucket]
    ...
    def process_S3_notifications(self, s3_conn, s3_notifications):
        ...
        for notification in s3_notifications:
            ...
            # These were validated in process_CT_notifications()
            ...
            for message in envelope['Records']:
                try:
                    bucket_name = message['s3']['bucket']['name']
                    key = message['s3']['object']['key']
                    if not message['eventName'].startswith('ObjectCreated'):
                        logger.log(logging.DEBUG, "eventName is not ObjectCreated: %s" % json.dumps(message))
                        continue
                except KeyError:
                    logger.log(logging.DEBUG, "Invalid S3 notification record: %s" % json.dumps(message))
                    continue
            ...
            if message_indexed and not message_failed:
    ...
    def process_CT_notifications(self, s3_conn, notifications):
        ...
        for notification in notifications:
            ...
            if all(key in envelope for key in ("s3Bucket", "s3ObjectKey")):
                logger.log(logging.DEBUG, "This is considered a CloudTrail message.")
                message=envelope
            elif 'Records' in envelope and \
                    isinstance(envelope['Records'],list) and \
                    envelope['Records'] and \
                    's3' in envelope['Records'][0] and \
                    'eventName' in envelope['Records'][0]:
                logger.log(logging.DEBUG, "This is considered a CloudTrail message from S3 bucket.")
                s3_notifications.append(notification)
                continue
            else:
                failed.append(notification)
                logger.log(logging.ERROR, "This doesn't look like a valid CloudTrail message. Please check SQS settings.")
        ...
        return completed, keys_to_delete, failed, s3_notifications
